...
client.on("message", (message) => {
  ...
  if (message.content.startsWith(prefix + "start")) {
  	...
  	if (/*forward or alias of forward*/) {
  		if(/*Enemy defeated?*/) {
        if (/*User.level = 5*/) {
          //User.AttackBoost = 1.2
          data.push(/*SecondaryText[User.Level]*/);
        } else if (/*User.level = 7*/) {
          //User.AttackBoost = 1.6
          data.push(/*SecondaryText[User.Level]*/);
        }
  			//User.level++
        data.push(/*text[User.level]*/);
        //User.EnemyHealth = EnemyHealths[User.Level]
  		}else{
